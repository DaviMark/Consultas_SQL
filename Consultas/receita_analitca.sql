...
FROM uolcs-datalake-engcorp-prd.`ecc_fi_sapphm_raw.OFIN08`
-- WHERE FD_PSTNGDATE >= '2025-01-01'
                           WHERE EXTRACT(YEAR FROM FD_PSTNGDATE) = 2024
...
SELECT
  ...
  ITEM_NUM,
  GL_ACCOUNT
FROM uolcs-datalake-engcorp-prd.`ecc_fi_sapphm_raw.OFIN02`
WHERE MATERIAL = '' AND EXTRACT(YEAR FROM FD_PSTNGDATE) = 2024
...
SELECT
  ...
  _BIC_CA_NUMREF,
  _BIC_CA_GRPREC
FROM uolcs-datalake-engcorp-prd.`ecc_sd_sapphm_raw._BIC_AOREC0100`
...
LEFT OUTER JOIN uolcs-datalake-engcorp-prd.`ecc_sd_sapphm_raw._BIC_AOSDF0200` AOSDF
...
SELECT
  ...
  DEBIT_LC,
  ACCOUNT,
FROM uolcs-datalake-engcorp-prd.`ecc_fi_sapphm_raw.OFIN03` FIN03
WHERE WHERE EXTRACT(YEAR FROM PSTNG_DATE) = 2024
...
SELECT
  ...
  MATERIAL,
FROM uolcs-datalake-engcorp-prd.`ecc_fi_sapphm_raw.OFIN02`
WHERE MATERIAL != '' AND EXTRACT(YEAR FROM PSTNG_DATE) = 2024
